...
if __name__ == "__main__":
    ...
    print "Usage"
    print "motion-template.py <video> [output] [n]"
    print "output is a path and a partial filename to append the frame number to, like '/tmp/frame-'"
    print "if n is present and is 1 output is a path in which per-frame directories will be created"
    print "ex: motion-template.py /tmp/a.avi /tmp/frame-"
    print "ex: motion-template.py /tmp/a.avi /tmp/ 1"
	...
	if( image ):
	    ...
	    if directories == 1:
		try:
		    os.mkdir(outfile + "/" + number)
		except OSError:
		    pass
		cvSaveImage(outfile + "/" + number + "/" + "motion-opencv.ppm", motion)

	    elif outfile == "":
		cvShowImage( "Motion", motion )
	    else:
		cvSaveImage( outfile + number + ".ppm", motion)
