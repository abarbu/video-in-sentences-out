...
int main(int argc, char* argv[])
{
  ...
  // featurePool = new CFeature2DPool(img_size, templ_size, 
  //                                  pyramid_level, mask_size, b_gpu, cam);
  // TC 2011-11-21 (Mon) -- don't use camera model
  featurePool = new CFeature2DPool (img_size, templ_size, 
                                    pyramid_level, mask_size, b_gpu, NULL);

	
  if (b_display) 
    init_track_display(disp_scale);

  //
  // Main loop
  //
  // const char *jpgFileNames_strA[2] = {"file1.jpg", "file2.jpg"};
  // const char *logFileNames_strA[2] = {"output-this.klt", "output-that.klt"};
  int ctime = 0, ptime = 0, frame = 0;
  while (b_run)
         // && (frame < 2))       // per-frame-klt takes just 2 frames
    {
      if (b_display) {
      ...
      // if (b_imu && ptime != 0) {
      //   if (!compute_homography_IMU(ptime, ctime, imu_list_name, 
      //                               imu_shape_name, 
      //                               cam->GetKK(KK), cam->GetKi(Ki), H)) 
      //     break;
      // }
      // TC 2011-11-21 (Mon) -- don't use IMU

      //
      // Step 3: Track and select the features
      //
      if (b_gpu) {
        featurePool->Track_GPU(frame, gimgf, H); 
        t1 = timer.Stop();
        featurePool->Select_GPU(frame, min_feature, max_feature, gimgf);
	t2 = timer.Stop();
      }
      else {
        featurePool->Track(frame, ctime, gimgf, H);	
        t1 = timer.Stop();
        featurePool->Select(frame, min_feature, max_feature, gimgf, false);
	t2 = timer.Stop();
      }
      ...
      t3 = timer.Stop();
      if (b_log) 
        featurePool->SaveToFile(0, track_log_name);
		
      //
      // Step 4: Display
      //
      if (b_display) run_track_display();	
      t4 = timer.Stop();

      CFeatureTrackStat& stat = featurePool->GetStat();
      printf("%3d, %8d %3d | %3d = %3d/%3d %3d |"
             " %5.1f = %5.1f + %5.1f + %5.1f, %5.1f\n",
             stat.frame, ctime-t_offset, (ctime-ptime), 
             stat.n_total, stat.n_success, 
             stat.n_total - stat.n_success, stat.n_new,
             (t3 - t0)*1e3, 
             (t1 - t0)*1e3, 
             (t2 - t1)*1e3, 
             (t3 - t2)*1e3, 
...
bool read_config_file(const char* fname)
{
  ...
  while (!fin.eof()) {
    ...
    if (str == "GPU") {
      ...
      fprintf (stderr, "Hi img_list_name is %s\n", img_list_name);
    }
