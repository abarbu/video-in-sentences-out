...
class CFeature2DPool : public std::vector<CFeature2DTrack*>
{
 public:
  ...
  void SaveToFile(int timestamp, const char* fname);
  void SaveToFile_tc (const char* fname); // TC 2011-11-22 (Tue) --custom save
  int SaveFrameResult_tc ();           // TC 2011-11-23 (Wed) -- update
  bool SaveAllToFile_tc (const char* fname); // TC 2011-11-22 (Tue) -- save all
  int GetFrameResult_tc (int frame);
  vector<float> GetFrameXresult_tc (int frame);
  vector<float> GetFrameYresult_tc (int frame);
  vector<int> GetFrameIdResult_tc (int frame);
 ...
 public:
  ...
  map<int, vector<int> > m_mapTcFeatureID;		//!< TC 2011-11-23 (Wed) --history of feature IDs
  map<int, vector<float> > m_mapTcFeatureX;		//!< TC 2011-11-23 (Wed) --history of feature X values
  map<int, vector<float> > m_mapTcFeatureY;		//!< TC 2011-11-23 (Wed) --history of feature Y values
