...
int playback(const char* fname, int t_offset, IplImage* gimgf)
{
	...
	if (getline(fin, str))
	{
		...
		IplImage* img = cvLoadImage(name.c_str());
                ...
                // TC 2011-11-22 (Tue) -- test by-passing smooth.
		...
		if (img->width != gimgf->width || img->height != gimgf->height) {
		  fprintf(stderr, "Error: Wrong image size, %d x %d\n", img->width, img->height);
			return -1;
		}
		...
		if (p == string::npos || q == string::npos) {
		  fprintf (stderr, "Error in reading timestamp from image file name ");
			return -1;
		}
...
void smooth_image_float(const IplImage* img, IplImage* gimgf, int w)
{
  ...
  // TC 2011-11-22 (Tue) -- dont use IPP
  fprintf (stderr, "Calling function '%s'\n", __FUNCTION__);
  fprintf (stderr, "TC 2011-11-22 (Tue) -- removed IPP dependence\n");
  exit (-1);
