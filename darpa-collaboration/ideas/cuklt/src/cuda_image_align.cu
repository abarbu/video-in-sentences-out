...
// #include <ipp.h>   // TC 2011-11-29 (Tue) --
...
// extern int templ_size; // TC 2011-12-01 (Thu) -- why use it??
...
int InitCUDA(int imageWidth, int imageHeight, int pyrLevel, int templateSize, int maxIteration, int maxLinesearch)
{
	...
	CUT_CHECK_ERROR("before Getting Device Count");
	cudaGetDeviceCount(&count);
	CUT_CHECK_ERROR("after Getting Device Count");
...
#if 0
	/* FIXME Andrei, why do we set the device here? */
	CUT_CHECK_ERROR("before setting device");
	cudaSetDevice(i);
	CUT_CHECK_ERROR("after setting device");
#endif
	...
	for (i=0;i < pyramidLevel;i++) // for each pyr level
	{
		...
		templSize[i] = templateSize; // TC 2011-12-01 (Thu) -- why use an extern templ_size variable?
...
bool ExitCUDA(void)
{
	...
	for (int i=0; i < pyramidLevel; i++)
	{
        ...
        cudaThreadExit ();      // TC 2011-12-07 (Wed) -- fix Setting the
                                // device when a process is active is not
                                // allowed.
	// fprintf (stderr, "successfully exited CUDA\n");
	return true;
}
							   ...
							   unsigned int width, unsigned int height)
{
  ...
  __shared__ float maxx[1280]; //  TC 2011-11-29 (Tue) -- make it able to
                               //  handle image dimension up to 1280
