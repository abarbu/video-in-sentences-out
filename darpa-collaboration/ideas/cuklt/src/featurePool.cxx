...
// #include <ipp.h>
// TC 2011-11-22 (Tue) -- remove IPP dependency
...
int CFeature2DPool::Add(float x, float y, float saliency, int frame, CImagePyramid* newPyr)
{
  ...
  int new_id = (int)size();
  // fprintf (stderr, "new id is %d\n", new_id) 
  // TC 2011-11-20 (Sun) --memory leak?
  ...
  for (list<int>::iterator it = listID.begin(); it != listID.end(); it++)
    {
      ...
      fsave << id << "\t";
      fsave << setprecision(2) << setiosflags(ios::fixed) << setw(12) 
            << fp->m_x;
      fsave << setprecision(2) << setiosflags(ios::fixed) << setw(12)
            << fp->m_y;
      fsave << setprecision(5) << setiosflags(ios::fixed) << setw(12)
            << fp->m_nx;
      fsave << setprecision(5) << setiosflags(ios::fixed) << setw(12) 
            << fp->m_ny;
      fsave << endl;
  ...
  for (list<int>::iterator it = listID.begin(); it != listID.end(); it++)
    {
      ...
      CFeature2D* fp = pTrack->back();
      idVec.push_back (id);
      xVec.push_back (fp->m_x);
      yVec.push_back (fp->m_y);
    }
  m_mapTcFeatureID[m_frame] = idVec;
  m_mapTcFeatureX[m_frame]  = xVec;
  m_mapTcFeatureY[m_frame]  = yVec;
  return listID.size ();
}

void CFeature2DPool::
SaveToFile_tc (const char* fname)
{
   ...
   */
  // fprintf (outFileP, "%d\n", listID.size());
  // TC 2011-11-25 (Fri) -- add keyword "nFeatures_v2 =" to make it
  // easier to detect format.
...
bool CFeature2DPool::
SaveAllToFile_tc (const char* fname)
{
  FILE *outFileP = fopen (fname, "wb");
  if (outFileP == NULL)
    {
   ...
   */
  int numFrames = m_mapTcFeatureID.size();
  fprintf (outFileP, "%d\n", numFrames);

  
  /*
   * Write features in each frames:
   */
  for (int fr_idx = 0; fr_idx < numFrames; fr_idx++)
    {
      vector<int> idVec = m_mapTcFeatureID[fr_idx];
       ...
       * write features:
       */
      vector<float> xVec =  m_mapTcFeatureX[fr_idx];
      vector<float> yVec =  m_mapTcFeatureY[fr_idx];
      int numFeatures = xVec.size ();
      for (int fe_idx = 0; fe_idx < numFeatures; fe_idx++)
        {
          fprintf (outFileP, "%d %.2f %.2f\n", 
                   idVec[fe_idx], xVec[fe_idx], yVec[fe_idx]);
        }
    }
...
int CFeature2DPool::
GetFrameResult_tc (int frame)
{
  vector<int> idVec = m_mapTcFeatureID [frame];
  return idVec.size ();
}

vector<float> CFeature2DPool::
GetFrameXresult_tc (int frame)
{
  // if (unlikely (frame > m_frame))
  if (unlikely ((frame > m_frame) || (frame < 0)))
    {
      // fprintf (stderr, "invalid frame number; %d > %d, use %d instead\n",
      //          frame, m_frame, m_frame);
      // frame = m_frame;
      fprintf (stderr, "featurePool.cxx: invalid frame number; %d > %d\n", 
               frame, m_frame);
      exit (EXIT_FAILURE);
    }
  // vector<float> xVec =  m_mapTcFeatureX[frame];
  // return xVec;                  // return a copy?
  return m_mapTcFeatureX[frame]; // return a copy?
}

vector<float> CFeature2DPool::
GetFrameYresult_tc (int frame)
{
  if (unlikely ((frame > m_frame) || (frame < 0)))
  // if (unlikely (frame > m_frame))
    {
  ...
  // vector<float> yVec =  m_mapTcFeatureY[frame];
  // return yVec;                  // return a copy?
  return m_mapTcFeatureY[frame]; // return a copy?
}


vector<int> CFeature2DPool::
GetFrameIdResult_tc (int frame)
{
  if (unlikely ((frame > m_frame) || (frame < 0)))
    {
      // fprintf (stderr, "invalid frame number; %d > %d, use %d instead\n",
  ...
  // vector<int> idVec = m_mapTcFeatureID [frame];
  // return idVec;                 // return a copy?
  return m_mapTcFeatureID[frame]; // return a copy?
}
