...
public:
 ...
 CFeature2D(float x, float y, int frame, int status = 0, 
            float rms = 0, float ncc = 0, float shrink = 0, float bias = 0,
            float nx = 0, float ny = 0, float x0 = 0, float y0 = 0) 
   /* : m_x(x), m_y(y), m_frame(frame), m_status(status), */
   /*  m_rms(rms), m_ncc(ncc), m_shear(shrink), m_bias(bias), */
   /*  m_nx(nx), m_ny(ny), m_x0(x0), m_y0(y0) {}; */
   // TC 2011-11-28 (Mon) -- recorder the initialization so the 
   // compiler will be happy
   : m_x0(x0), m_y0(y0), m_x(x), m_y(y), m_nx(nx), m_ny(ny),
    m_frame(frame), m_status(status),
    m_rms(rms), m_ncc(ncc), m_shear(shrink), m_bias(bias) {};
