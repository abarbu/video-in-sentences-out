...
%
%   OPTIONAL INPUT
%   Argument places 4 to 6 are reserved for optional input. The three
...
%
%       % Fit a spline of order 4 with 8 pieces and constraints
...
% Check input
[x,y,dim,breaks,n,periodic,constr] = arguments(varargin{:});
...
% Bin data
[junk,ibin] = histc(x,[-inf,breaks(2:end-1),inf]);
...
coefs = sparse(ii,jj,base.coefs,pieces+n-1,n*pieces);
coefs = u*coefs;
...
%--------------------------------------------------------------------------
function [x,y,dim,breaks,n,periodic,constr] = arguments(varargin)
...
% Count arguments
if nargin < 3
    msgid = 'arguments:nargin1';
    error(msgid,'Not enough input arguments.')
elseif nargin > 6
    msgid = 'arguments:nargin2';
    error(msgid,'Too many input arguments.')
end
...
% Check length of x
if mx == 0
    msgid = 'arguments:nodata';
    error(msgid,'Vector x must not be empty.')
end

% Remove trailing singleton dimensions from y-data
y = varargin{2};
dim = size(y);
...
if isscalar(varargin{3})
    ...
    if x(1) < x(end)
        ...
        [junk,ibin] = histc(ibreaks,[0,2:mx-1,mx+1]);
    ...
    if min(breaks) == max(breaks)
        msgid = 'arguments:breaks2';
        error(msgid,'At least two unique breaks are required.');
    end
end
...
for k = 4:nargin
    ...
    elseif isstruct(a) && isscalar(a)
        ...
        msgid = 'arguments:nonsense';
        error(msgid,'Failed to interpret argument #%d.',k)
    end
end
...
if x(1) < xlim1 || x(end) > xlim2
    if periodic
        ...
        msgid = 'arguments:extdata';
        warning(msgid,'Some data are outside the spline domain.')
    end
end
...
yc = [];
cc = [];
if isfield(constr,'xc'), xc = constr.xc; end
if isfield(constr,'yc'), yc = constr.yc; end
if isfield(constr,'cc'), cc = constr.cc; end
...
% Check yc
if isempty(yc)
    yc = zeros(prod(dim),nx);
elseif numel(yc) ~= prod(dim)*nx
    msgid = 'arguments:ycsize';
    error(msgid,'Cannot reshape yc to size %dx%d.',prod(dim),nx)
else
    % Reshape yc
    yc = reshape(yc,[],nx);
end
...
elseif numel(size(cc)) ~= 2
    msgid = 'arguments:ccsize1';
    error(msgid,'Constraint coefficients cc must be 2D.')
elseif size(cc,2) ~= nx
    msgid = 'arguments:ccsize2';
    error(msgid,'Last dimension of cc must equal length of xc.')
end

% Check high order derivatives
if size(cc,1) >= n
    if any(any(cc(n:end,:)))
        msgid = 'arguments:difforder';
        mess = 'Constraints involve derivatives of order %d or larger.';
        error(msgid,mess,n-1)
    end
    cc = cc(1:n-1,:);
end
...
if nz + ncon > m2
	msgid = 'solvecon:deficient';
	warning(msgid,'Rank deficient constraints, rank = %d.',m2-nz);
end
