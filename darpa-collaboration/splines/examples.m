...
% Uniform breaks (65 breaks, 64 pieces)
breaks = linspace(0,2*pi,65);
pp1 = splinefit(x,y,breaks);

% Breaks interpolated from data (8 pieces)
pp2 = splinefit(x,y,8);
...
legend('data','64 pieces','8 pieces')
...
x = cumsum(rand(1,99));
x = 2*pi*[0, x/max(x)];
y = sin(x) - cos(2*x) + 0.3*randn(size(x));
